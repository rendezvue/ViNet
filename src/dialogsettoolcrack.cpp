...
void DialogSetToolCrack::showEvent(QShowEvent *ev)
{
	...
	//Get Level 
    int inspect_level = CEnsemble::getInstance()->m_cls_api.Ensemble_Tool_Option_Crack_Get_InspectLevel(GetId());
	//Set Slider
...
void DialogSetToolCrack::OnSliderSetFeatureLevel(void)
{
	...
	//set level
    CEnsemble::getInstance()->m_cls_api.Ensemble_Tool_Option_Crack_Set_InspectLevel(GetId(), level);
