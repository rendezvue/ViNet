...
void DialogSetToolAngleInspect::showEvent(QShowEvent *ev)
{
	...
	//ID
	std::string str_id_base = CEnsemble::getInstance()->GetSelectDevice()->Ensemble_Tool_Option_InspectAngle_Get_ID_Info_Base(GetId()) ;
	std::string str_id_target = CEnsemble::getInstance()->GetSelectDevice()->Ensemble_Tool_Option_InspectAngle_Get_ID_Info_Target(GetId()) ;
...
{
	//
	float angle = CEnsemble::getInstance()->GetSelectDevice()->Ensemble_Tool_Angle_Get_CalcAngle(GetParentId()) ;
...
void DialogSetToolAngleInspect::OnButtonGetTolerance(void)
{
	...
	CEnsemble::getInstance()->GetSelectDevice()->Ensemble_Tool_Option_InspectAngle_Get_Tolerance(GetId(), &tol_min, &tol_max) ;
...
void DialogSetToolAngleInspect::OnButtonSetTolerance(void)
{
	...
	CEnsemble::getInstance()->GetSelectDevice()->Ensemble_Tool_Option_InspectAngle_Set_Tolerance(GetId(), min, max) ;
