    ...
    ui(new Ui::dialogcheckforupdates)
{
    ...
    string cur_version = CEnsemble::getInstance()->GetSelectDevice()->Ensemble_Update_Get_Current_Version();
...
{
    string version_strings = CEnsemble::getInstance()->GetSelectDevice()->Ensemble_Update_Get_Version_List();
...
void dialogcheckforupdates::on_pushButton_Apply_Restart_clicked()
{
    ...
    {
        CEnsemble::getInstance()->GetSelectDevice()->Ensemble_Update_Set_Version(select_Version.toStdString());
        this->close();
    }
}
